...
/*typ=1 armator, 2 = wlasciciel ladunku*/
...
CREATE  TABLE `databaseheaven`.`Kontrahent` (
  ...
  CREATE TABLE `databaseheaven`.`Port` (
  ...
  FOREIGN KEY (`id_Port`) REFERENCES Port(`id_Port`) ON DELETE CASCADE,
...
/*  ENUM('Celna (Cło)', 'Celna (Kara)', 'Celna (Inna)', 'Portowa (za dok)', 'Portowa (za magazyn)', 'Portowa (inna)' )*/
