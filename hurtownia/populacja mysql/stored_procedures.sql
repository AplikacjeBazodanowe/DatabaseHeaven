...
root:BEGIN
    ...
    SELECT COUNT(*) FROM Oddokowany o
        WHERE o.id_zadokowany = zadok 
        INTO czyJuzIstnieje;
    ...
    IF iloscLadunkow <= iloscPozytywnychKontroliCelnych THEN
       ...
       IF oplataZaDokowanie > 0 THEN
		   INSERT INTO Oplata(typ, kwota, czy_Oplacona, data_naliczenia, id_kontrahent, id_Uzytkownik, id_Zadokowany) 
				VALUES (1, oplataZaDokowanie, 0, data, kontrh, user, zadok);
		END IF;
...
DELIMITER //
CREATE PROCEDURE odbior(cargo INT, recipient INT, user INT, data DATETIME, remarks TEXT)
root:BEGIN
    ...
    INSERT INTO Przeladunek VALUES (NULL, data, NULL, 1, shipID, NULL, storeID, NULL, user, cargo);   
    ...
    INSERT INTO Odbior_Ladunku VALUES(NULL, data, recipient, cargo);
	SET cost=0;            
	SET oldStore=NULL;            
    OPEN cur1;
    read_loop: LOOP
	...
	IF cost>0 THEN
		INSERT INTO Oplata(typ, kwota, czy_oplacona, data_naliczenia, id_kontrahent, id_Uzytkownik, id_Ladunek)
		            VALUES(1, cost, 0, data, recipient, user, cargo);
    END IF;
...
DELIMITER //
CREATE PROCEDURE nadanie(commodity INT, amount INT, sender INT, dutyControl BOOLEAN , ship INT, store INT, user INT, data DATETIME, remarks TEXT)
root:BEGIN
	...
	INSERT INTO Nadanie_Ladunku VALUES(NULL,data,sender,newID);	
	CALL przeladuj(newID, ship, store, user, data, remarks,TRUE);
...
DELIMITER //
CREATE PROCEDURE przeladuj(cargo INT, ship INT, store INT, user INT, data DATETIME, remarks TEXT, ignoreDuty BOOLEAN)
root:BEGIN
    ...
    IF ship IS NOT NULL THEN
        ...
        IF sumVol <= destMaxVol - destVol AND sumWeight <= destMaxWeight - destWeight THEN
            UPDATE Przeladunek SET czy_aktualne_polozenie = 0 WHERE id_Przeladunek = loadID;
	        INSERT INTO Przeladunek	VALUES (NULL, data, remarks, 1, sourceShip, ship, sourceStore, NULL, user, cargo);
    ...
    ELSEIF store IS NOT NULL THEN
        ...
        IF sumVol <= destMaxVol - destVol THEN
            START TRANSACTION;
            UPDATE Przeladunek SET czy_aktualne_polozenie = 0 WHERE id_Przeladunek = loadID;
	        INSERT INTO Przeladunek	VALUES (NULL, data, remarks, 1, sourceShip, NULL, sourceStore, store, user, cargo);
...
FOR EACH ROW
root:BEGIN
    DECLARE user, cust, cost, positiv, cargo INT;
    
    SET positiv = NEW.czy_pozytywna;
    SET cargo = NEW.id_Ladunek;
    ...
    INSERT INTO Oplata(typ, kwota, czy_oplacona, data_naliczenia, id_kontrahent, id_Uzytkownik, id_Ladunek)
                VALUES(2, cost, 0, NEW.data, cust, user,cargo);
END
|
delimiter ;
...
delimiter |
CREATE PROCEDURE konfiskata(cargo INT, remarks TEXT, user INT, data DATETIME)
BEGIN
	DECLARE ship_id, term_id INT;
	DECLARE sumWeight, sumVol FLOAT;
	START TRANSACTION;
	SELECT id_Statek2, id_Terminal 
		FROM Przeladunek 
			INNER JOIN Ladunek USING(id_Ladunek) 
			INNER JOIN Zadokowany ON Przeladunek.id_Statek2=Zadokowany.id_Statek
			INNER JOIN Dok USING(id_Dok)
		WHERE id_Ladunek=cargo AND czy_aktualne_polozenie=TRUE
		ORDER BY RAND() LIMIT 1
	INTO ship_id, term_id;
	SELECT (t.masa_jednostkowa *l.ilosc) FROM Ladunek l
        INNER JOIN Towar t ON t.id_Towar = l.id_Towar
        WHERE l.id_Ladunek = cargo INTO sumWeight;
    SELECT (t.objetosc_jednostkowa *l.ilosc) FROM Ladunek l
        INNER JOIN Towar t ON t.id_Towar = l.id_Towar
        WHERE l.id_Ladunek = cargo INTO sumVol;   
	INSERT INTO Kontrola_Celna VALUES(NULL,remarks,FALSE,data,user,cargo,term_id);
	...
	INSERT INTO Przeladunek VALUES(NULL,data,NULL,TRUE,ship_id,NULL,NULL,NULL,user,cargo);	
	COMMIT; 
END
|
delimiter ;
